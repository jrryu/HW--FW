...
/* Hearing plugin task {TaskData, cvc_plugin_variant, encoder, two_mic, sco_config, adc_dac_16kHz} */
const CvcPluginTaskdata alango_1mic_hearing_plugin = {{message_handler},DSP_HEARING, LINK_ENCODING_CVSD, 0, AUDIO_CODEC_CVSD, 1};
const CvcPluginTaskdata alango_2mic_hearing_plugin = {{message_handler},DSP_HEARING, LINK_ENCODING_CVSD, 1, AUDIO_CODEC_CVSD, 1};
...
static void handleInternalMessage ( Task task , MessageId id, Message message ) 	
{
	switch (id)
	{
		...
		case MESSAGE_HEARING_CHANGE_PARAMS:
		{
			AlangoPluginChangeParamsHearing((CvcPluginTaskdata*)task);
		}
		break;
