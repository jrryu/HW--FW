...
DESCRIPTION
	Helper function to increase or decrease Hearing volume based on direction
	parameter.

RETURNS
	TRUE if volume adjusted, FALSE if no streams found   
*/
static bool VolumeCheckHearing(volume_direction dir)    
{
    ...
    if (theSink.HearPhones_conf->Hearing.ActiveHearing)
    {
        switch(dir)
        {
			case increase_volume:
			{
            	if (theSink.HearPhones_conf->Hearing.volume < 0xf)
				{
					theSink.HearPhones_conf->Hearing.volume++;
					AudioSetVolume ( (uint16)theSink.HearPhones_conf->Hearing.volume, TonesGetToneVolume(TRUE), theSink.codec_task );
					TonesPlayTone(0x06, TRUE, TRUE);
				}
				else
					TonesPlayTone(0x07, TRUE, TRUE);
			} break;
			case decrease_volume:
			{
	  			if (theSink.HearPhones_conf->Hearing.volume > 0)
				{
					theSink.HearPhones_conf->Hearing.volume--;
	  				AudioSetVolume ( (uint16)theSink.HearPhones_conf->Hearing.volume, TonesGetToneVolume(TRUE), theSink.codec_task );
					TonesPlayTone(0x02, TRUE, TRUE);
				}
				else
					TonesPlayTone(0x01, TRUE, TRUE);
			} break;
			default: /* same volume was selected */
			{
				AudioSetVolume ( (uint16)theSink.HearPhones_conf->Hearing.volume, TonesGetToneVolume(TRUE), theSink.codec_task );
			} break;
		}
