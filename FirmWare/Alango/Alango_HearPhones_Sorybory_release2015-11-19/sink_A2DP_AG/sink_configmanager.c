...
{ 
	...
	/* Allocate memory for Alango's HearPhones configuration */
	theSink.HearPhones_conf = mallocPanic( sizeof(config_hearphones_t) );
    memset(theSink.HearPhones_conf, 0, sizeof (config_hearphones_t));
	CONF_DEBUG(("INIT: Malloc size HearPhones: [%d]\n",sizeof(config_hearphones_t)));
}
...
*/ 
void configManagerWriteHearPhonesData( void )
{
    config_hearphones_t sub_config ;
    ...
    CONF_DEBUG(("CONF : PS Write Vol Inverted[%c], LED Disable [%c], Multipoint Enable [%c], LBIPM Enable [%c] Enhancements[%x]\n" , 
                        ...
                        theSink.lbipmEnable ? 'T':'F',
                        lTemp.audio_enhancements));*/
    sub_config = *theSink.HearPhones_conf;
    (void) ConfigStore(CONFIG_HEARPHONES, &sub_config, sizeof(config_hearphones_t)); 
    
}

/****************************************************************************
NAME 
  	configManagerGetHearPhonesConfig

DESCRIPTION
    Reads the PSKEY containing Alango's HearPhones configuration
 
RETURNS
  	TRUE if configuration data was retrieved from CONFIG_HEARPHONES
*/

bool configManagerGetHearPhonesConfig( void )
{
    uint16				size;
    config_hearphones_t	sub_config;
	
    
    size = ConfigRetrieve(CONFIG_HEARPHONES, (void *)&sub_config, sizeof(config_hearphones_t));
    if ( size == sizeof(config_hearphones_t) )
    {
		CONF_DEBUG(("CONF: Hearing Profile[%x]\n	# of Hearing Profiles[%x]\n	Hearing EQ[%x]\n	# of Hearing Eqs[%x]\n	Hearing Volume[%x]\n",
							sub_config.Hearing.profile,
							sub_config.Hearing.Num_Of_Profiles,
							sub_config.Hearing.EQ,
							sub_config.Hearing.Num_Of_EQ,
							sub_config.Hearing.volume));
        theSink.HearPhones_conf->Hearing	= sub_config.Hearing;
#ifdef ENABLE_AG
		theSink.HearPhones_conf->Ag			= sub_config.Ag;
#endif
        return TRUE;
    }
    else
    {
        CONF_DEBUG(("CONF: HearPhones config read failed (or PS data not present)\n"));
        memset(theSink.HearPhones_conf, 0, sizeof(config_hearphones_t)); /* Zeroing all HearPhones configurations */
		/* Changing specific Hearing configuration to non-zero default values */
		theSink.HearPhones_conf->Hearing.Num_Of_Profiles = 1;
		theSink.HearPhones_conf->Hearing.volume = 11;
#ifdef ENABLE_AG
		theSink.HearPhones_conf->Ag.AGvolume	= 11;
#endif
...
/****************************************************************************
NAME 
  	configManagerWriteAGBdaddr

DESCRIPTION
    Writes the HeaPhones AG BDADDR to PSKEY CONFIG_HEARPHONES
...
*/
#ifdef ENABLE_AG
bool configManagerWriteAGBdaddr( const bdaddr * addr )
{
    config_hearphones_t   sub_config;
    uint16              size;

    /* DO NOT modify the other params on that PS KEY, they must remain unchanged */
    sub_config.Hearing	= theSink.HearPhones_conf->Hearing;
	sub_config.Ag		= theSink.HearPhones_conf->Ag;
    
    /* Populate the Bluetooth address to be written */
    if (addr == NULL)
    {
        /* No address passed in, so zero pad the addr part of the key */
        memset(&sub_config.Ag.ag_bd_addr, 0, sizeof(bdaddr));
    }
    else
    {
        memcpy(&sub_config.Ag.ag_bd_addr, addr, sizeof(bdaddr));
    }
    
    /* Store the AG Bluetooth address to PS */
    size = ConfigStore(CONFIG_HEARPHONES, &sub_config, sizeof(sub_config));
    if (size == sizeof(sub_config))
    {
        CONF_DEBUG(("CONF: Wrote sub bdaddr to CONFIG_HEARPHONES\n"));
        return TRUE;
    }
    else
    {
        CONF_DEBUG(("CONF: Error writing CONFIG_HEARPHONES\n"))
        return FALSE;
    }
	return 0;
}
...
{     
    ...
    configManagerAudioPromptEvents( keyLengths->num_audio_prompt_events ) ;
		...
		/* read the Alango's HearPhones pskey configuration */
		configManagerGetHearPhonesConfig();
    }
