...
#include <panic.h>
#include <stream.h>
/*#define DEBUG_PRINT_ENABLEDx*/ /*Mandatory for PRINT() to work!*/
#include <print.h>
#include <kalimba.h>
#include <file.h>
...
#include "sink_private.h"			/*for hearing parameters structure*/
#include "VM_HEP_config.h"          /*for hearing defines*/

/* ptsp for alango audio recording */
#define LOG_PIO     1
#define LOG_MASK    ((1 << LOG_PIO) & 0xFFFF)
...
#endif
static void HearingCodecMessage (CvcPluginTaskdata *task, T_mic_gain input_gain_l, T_mic_gain input_gain_r, uint16 output_gain ) ;

/*void ConnectStreamsHearing(void);
uint8 CsrExamplePluginChangeEQ( uint8 Value );
bool CsrExamplePluginGetVolumeHearing( uint16 * Value );*/                                                                                
...
typedef struct audio_Tag
{
	...
	/* variables used for hearing operation */
	
	/*Hearing Word 1*/
	/*unsigned DSP_Max_Volume:8;
    unsigned DSP_Min_Volume:8;*/
	/*Hearing Word 2*/
    unsigned profile:4;
    unsigned EQ:4;
    unsigned Num_Of_Profiles:4;
    unsigned Num_Of_EQ:4;
	/*Hearing Word 3*/
    unsigned EQ_Load_Flag:1;
	unsigned HearingMode:1; /* flag used to control the connecting of hearing mode */
	unsigned HearingWaitToneStart:1;
	unsigned Hearing_Busy_Flag:1;
...
const uint16 ccoefs[10] = {2047, -1024, 0x0000, -988, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,0x0000};
...
static const char* csrCvcPluginGetKapFile(CVC_PLUGIN_TYPE_T variant)
{
    ...
    {
        case CVSD_CVC_1_MIC_HEADSET:
			return "VCP_headset/vcp_csr8670_1mic_cvsd.kap";
        case CVSD_CVC_1_MIC_HEADSET_BEX:
        case WBS_CVC_1_MIC_HEADSET:
        case CVSD_CVC_1_MIC_HEADSET_ASR:
            /*return "cvc_headset/cvc_headset.kap";*/
			return "VCP_headset_wbs/vcp_csr8670_1mic_wbs.kap";
			
        case CVSD_CVC_2_MIC_HEADSET:
			return "VCP_headset_2mic/vcp_csr8670_2mic_cvsd.kap";
        case CVSD_CVC_2_MIC_HEADSET_BEX:
        case WBS_CVC_2_MIC_HEADSET:
        case CVSD_CVC_2_MIC_HEADSET_ASR:
            /*return "cvc_headset_2mic/cvc_headset_2mic.kap";*/
			return "VCP_headset_2mic_wbs/vcp_csr8670_2mic_wbs.kap";
		...
		case DSP_HEARING:
			return "hearing/khep_MA_Analouge_1MIC_16Khz.kap";
        default:
            Panic() ;
            return NULL;
    }
                          ...
                          TaskData * app_task)
{
    ...
    if (task->cvc_plugin_variant == DSP_HEARING)
    {
        CVC->HearingMode = 1;
        CVC->volume             = theSink.HearPhones_conf->Hearing.volume;
        CVC->profile            = theSink.HearPhones_conf->Hearing.profile;
        CVC->Num_Of_Profiles    = theSink.HearPhones_conf->Hearing.Num_Of_Profiles;
        CVC->EQ                 = theSink.HearPhones_conf->Hearing.EQ;
        CVC->Num_Of_EQ          = theSink.HearPhones_conf->Hearing.Num_Of_EQ;   
        CVC->EQ_Load_Flag       = FALSE;
        CVC->Hearing_Busy_Flag  = FALSE;
    }
    if (!CVC->HearingMode)
    {
        /* Set clear mic pin */
...
#ifdef LOG_PIO
	PRINT(("Alango RECORDING START !!!\n"));
    PioSetKalimbaControl32(LOG_MASK, LOG_MASK);
#endif
    ...
    if (CVC->HearingMode)
        PanicFalse( KalimbaSendMessage(KALIMBA_MSG_GO,0,0,0,0) ); /* Have to send KALIMBA_MSG_GO in order to start Hearing DSP */
...
void CsrCvcPluginDisconnect( CvcPluginTaskdata *task )
{
	...
	/* Disconnect streams for Alango's Hearing mode */
	else if (task->cvc_plugin_variant == DSP_HEARING)
	{
		Source mic_source_a = NULL;
        Source mic_source_b = NULL;
		Source mic_source_c = NULL;
		Source mic_source_d = NULL;
		...
		/* disconnect mic channels */
		if (CVC->tone_stereo == 0 && task->two_mic == 0)
        {
			mic_source_a = StreamAudioSource(CVC->digital->mic_a.digital ? AUDIO_HARDWARE_DIGITAL_MIC : AUDIO_HARDWARE_CODEC, 
												CVC->digital->mic_a.digital ? AUDIO_INSTANCE_1 : AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
			StreamDisconnect(mic_source_a, StreamKalimbaSink(0));  
			SourceClose(mic_source_a);
		}
		else if (CVC->tone_stereo == 0 && task->two_mic == 1)
        {
			mic_source_a = StreamAudioSource(CVC->digital->mic_a.digital ? AUDIO_HARDWARE_DIGITAL_MIC : AUDIO_HARDWARE_CODEC,
												CVC->digital->mic_a.digital ? AUDIO_INSTANCE_1 : AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
			mic_source_b = StreamAudioSource(CVC->digital->mic_b.digital ? AUDIO_HARDWARE_DIGITAL_MIC : AUDIO_HARDWARE_CODEC, 
												CVC->digital->mic_b.digital ? AUDIO_INSTANCE_1 : AUDIO_INSTANCE_0, AUDIO_CHANNEL_B);
			StreamDisconnect(mic_source_a, StreamKalimbaSink(0));
			StreamDisconnect(mic_source_b, StreamKalimbaSink(1));
			SourceClose(mic_source_a);
			SourceClose(mic_source_b);
		}
		else if (CVC->tone_stereo == 1 && task->two_mic == 0)
		{
			mic_source_a = StreamAudioSource(CVC->digital->mic_a.digital ? AUDIO_HARDWARE_DIGITAL_MIC : AUDIO_HARDWARE_CODEC,
												AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
			mic_source_c = StreamAudioSource(CVC->digital->mic_b.digital ? AUDIO_HARDWARE_DIGITAL_MIC : AUDIO_HARDWARE_CODEC,
												CVC->digital->mic_b.digital ? AUDIO_INSTANCE_1 : AUDIO_INSTANCE_0,
												CVC->digital->mic_b.digital ? AUDIO_CHANNEL_A : AUDIO_CHANNEL_B);
			StreamDisconnect(mic_source_a, StreamKalimbaSink(0));
			StreamDisconnect(mic_source_c, StreamKalimbaSink(1));
			SourceClose(mic_source_a);
			SourceClose(mic_source_c);
		}
		else if (CVC->tone_stereo == 1 && task->two_mic == 1)
		{
			mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
			mic_source_b = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_B);
			mic_source_c = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_A);
			mic_source_d = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_B);
			StreamDisconnect(mic_source_a, StreamKalimbaSink(0));
			StreamDisconnect(mic_source_b, StreamKalimbaSink(1));  
			StreamDisconnect(mic_source_c, StreamKalimbaSink(2));  
			StreamDisconnect(mic_source_d, StreamKalimbaSink(4));
			SourceClose(mic_source_a);
			SourceClose(mic_source_b);
			SourceClose(mic_source_c);
			SourceClose(mic_source_d);
		}
		PRINT(("HEP: Disconnected\n"));
	}
    else
    {
        Source mic_source_a = NULL;
...
{
    uint32 sample_rate;
	uint16 *coefs =  PanicUnlessMalloc(sizeof(ccoefs));

	memcpy(coefs, ccoefs, sizeof(ccoefs)); /* copying from constants memory region the IIR coefs */	
        ...
        (task->cvc_plugin_variant == CVSD_CVC_1_MIC_HANDSFREE_ASR)||(task->cvc_plugin_variant == CVSD_CVC_2_MIC_HANDSFREE_ASR))
    {
        ...
        switch(CVC->features.audio_input_routing)
        {
			case AUDIO_ROUTE_INTERNAL:
				...
				/* Used DC removal IIR: (1 - z^(-1))  /  (  1 - 0.965*z^(-1)  ), 0.965 @ 48 KHz Fs ==> Fc = 150 Hz, 0.965 of -1024 ~= -988 */
				PanicFalse(CodecSetIirFilter(AUDIO_INSTANCE_0, AUDIO_CHANNEL_A ,TRUE, coefs ));
				...
				if( task->two_mic )
				{
					...
					/* Used DC removal IIR: (1 - z^(-1))  /  (  1 - 0.965*z^(-1)  ), 0.965 @ 48 KHz Fs ==> Fc = 150 Hz, 0.965 of -1024 ~= -988 */
					PanicFalse(CodecSetIirFilter(AUDIO_INSTANCE_0, AUDIO_CHANNEL_B ,TRUE, coefs ));
	...
	/* connect streams for Alango's Hearing mode */
	else if(task->cvc_plugin_variant == DSP_HEARING)
    {
        /*uint16 kal_port_a = CVC_LEFT_PORT;
        uint16 kal_port_b = CVC_RIGHT_PORT;*/
        ...
        PanicFalse( StreamConnect(StreamKalimbaSource(0), spkr_sink_a) );
        /* Plug port 1 into Right DAC */
        PanicFalse( StreamConnect(StreamKalimbaSource(1), spkr_sink_b) );
        
#if defined(DIGITAL_MICS)
        /* Checking if Hearing definde to work with Mono and 1 Mic */
        if (CVC->tone_stereo == 0 && task->two_mic == 0)
        {
            mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_A);
            MicbiasConfigure(MIC_BIAS_0,MIC_BIAS_VOLTAGE,1);
            MicbiasConfigure(MIC_BIAS_0,MIC_BIAS_ENABLE,MIC_BIAS_FORCE_ON);
            SourceConfigure(mic_source_a, STREAM_DIGITAL_MIC_INPUT_RATE, adcrate);
			
			/* Used DC removal IIR: (1 - z^(-1))  /  (  1 - 0.965*z^(-1)  ), 0.965 @ 48 KHz Fs ==> Fc = 150 Hz, 0.965 of -1024 ~= -988 */
			PanicFalse(CodecSetIirFilter(AUDIO_INSTANCE_1, AUDIO_CHANNEL_A ,TRUE, coefs ));
			
            PanicFalse(StreamConnect(mic_source_a ,StreamKalimbaSink(0)));
        }
        else if (CVC->tone_stereo == 0 && task->two_mic == 1)
        {
            mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_A);
            mic_source_b = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_B);
            ...
            SourceConfigure(mic_source_a, STREAM_DIGITAL_MIC_INPUT_RATE, adcrate);
            SourceConfigure(mic_source_b, STREAM_DIGITAL_MIC_INPUT_RATE, adcrate);
			
			/* Used DC removal IIR: (1 - z^(-1))  /  (  1 - 0.965*z^(-1)  ), 0.965 @ 48 KHz Fs ==> Fc = 150 Hz, 0.965 of -1024 ~= -988 */
			PanicFalse(CodecSetIirFilter(AUDIO_INSTANCE_1, AUDIO_CHANNEL_A ,TRUE, coefs ));
			PanicFalse(CodecSetIirFilter(AUDIO_INSTANCE_1, AUDIO_CHANNEL_B ,TRUE, coefs ));
			
            SourceSynchronise(mic_source_a, mic_source_b); 
            PanicFalse(StreamConnect(mic_source_a ,StreamKalimbaSink(0)));
            PanicFalse(StreamConnect(mic_source_b ,StreamKalimbaSink(1)));
        }
        else if (CVC->tone_stereo == 1 && task->two_mic == 0)
        {
            mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
            ...
            MicbiasConfigure(MIC_BIAS_1,MIC_BIAS_ENABLE,MIC_BIAS_FORCE_ON);  
            SourceConfigure(mic_source_a, STREAM_DIGITAL_MIC_INPUT_RATE, adcrate);
            SourceConfigure(mic_source_c, STREAM_DIGITAL_MIC_INPUT_RATE, adcrate);
			
			/* Used DC removal IIR: (1 - z^(-1))  /  (  1 - 0.965*z^(-1)  ), 0.965 @ 48 KHz Fs ==> Fc = 150 Hz, 0.965 of -1024 ~= -988 */
			PanicFalse(CodecSetIirFilter(AUDIO_INSTANCE_0, AUDIO_CHANNEL_A ,TRUE, coefs ));
			PanicFalse(CodecSetIirFilter(AUDIO_INSTANCE_1, AUDIO_CHANNEL_A ,TRUE, coefs )); 
    
            SourceSynchronise(mic_source_a, mic_source_c); 
            PanicFalse(StreamConnect(mic_source_a ,StreamKalimbaSink(0)));
            PanicFalse(StreamConnect(mic_source_c ,StreamKalimbaSink(1)));                
        }
        else if (CVC->tone_stereo == 1 && task->two_mic == 1)
        {
            mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
            ...
            SourceConfigure(mic_source_b, STREAM_DIGITAL_MIC_INPUT_RATE, adcrate);
            SourceConfigure(mic_source_c, STREAM_DIGITAL_MIC_INPUT_RATE, adcrate);
            SourceConfigure(mic_source_d, STREAM_DIGITAL_MIC_INPUT_RATE, adcrate);
            ...
            SourceSynchronise(mic_source_a, mic_source_b);
            SourceSynchronise(mic_source_a, mic_source_c);
            SourceSynchronise(mic_source_a, mic_source_d);
    
            PanicFalse(StreamConnect(mic_source_a ,StreamKalimbaSink(0)));
            PanicFalse(StreamConnect(mic_source_b ,StreamKalimbaSink(1))); 
            PanicFalse(StreamConnect(mic_source_c ,StreamKalimbaSink(2)));
            PanicFalse(StreamConnect(mic_source_d ,StreamKalimbaSink(4)));
         }
#else
		
		mic_source_a = StreamAudioSource( AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A );
		PanicFalse( SourceConfigure(mic_source_a, STREAM_CODEC_INPUT_RATE, 16000) );
		PanicFalse( SourceConfigure(mic_source_a, STREAM_CODEC_MIC_INPUT_GAIN_ENABLE, 1) );
    	PanicFalse(MicbiasConfigure(MIC_BIAS_0, MIC_BIAS_ENABLE, MIC_BIAS_FORCE_ON));
		
		if (CVC->tone_stereo == 1 || task->two_mic == 1)
		{
    		mic_source_b = StreamAudioSource( AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_B );
			PanicFalse( SourceConfigure(mic_source_b, STREAM_CODEC_INPUT_RATE, 16000) );
			PanicFalse( SourceSynchronise(mic_source_a, mic_source_b) );
			PanicFalse( SourceConfigure(mic_source_b, STREAM_CODEC_MIC_INPUT_GAIN_ENABLE, 1) );
...
/****************************************************************************
DESCRIPTION
    Handles a CVC_CODEC message received from Hearing
*/
static void HearingCodecMessage (CvcPluginTaskdata *task, T_mic_gain input_gain_l, T_mic_gain input_gain_r, uint16 output_gain )
{
	Source mic_source_a = NULL;
	...
	Source mic_source_d = NULL;
#endif
	uint16 linput_gain_l = *(uint16*)&input_gain_l & 0xf;
	uint16 linput_gain_r = *(uint16*)&input_gain_r & 0xf;
	
	PRINT(("HEP: Output gain = 0x%x\n" , output_gain ));
    PRINT(("HEP: Input gain L:R = 0x%x : 0x%x \n", *(uint16*)&input_gain_l,*(uint16*)&input_gain_r));
    ...
    if(CVC)
    {
        ...
        /* Set input gain(s) */
#if defined(DIGITAL_MICS)
        /* Checking if Hearing defined to work with Mono and 1 Mic */
        if (CVC->tone_stereo == 0 && task->two_mic == 0)
        {
            mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_A);
            SourceConfigure(mic_source_a, STREAM_DIGITAL_MIC_INPUT_GAIN, (linput_gain_l & 0xf));
        }
		/* Checking if Hearing defined to work with Mono and 2 Mics */
        else if (CVC->tone_stereo == 0 && task->two_mic == 1)
        {
            mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_A);
            mic_source_b = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_B);
			SourceConfigure(mic_source_a, STREAM_DIGITAL_MIC_INPUT_GAIN, (linput_gain_l & 0xf));
            SourceConfigure(mic_source_b, STREAM_DIGITAL_MIC_INPUT_GAIN, (linput_gain_l & 0xf));
        }
		/* Checking if Hearing defined to work with Stereo and 1 Mic at every side */
        else if (CVC->tone_stereo == 1 && task->two_mic == 0)
        {
            mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
            mic_source_c = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_1, AUDIO_CHANNEL_A);
			SourceConfigure(mic_source_a, STREAM_DIGITAL_MIC_INPUT_GAIN, (linput_gain_l & 0xf));
            SourceConfigure(mic_source_c, STREAM_DIGITAL_MIC_INPUT_GAIN, (linput_gain_r & 0xf));
        }
		/* Checking if Hearing defined to work with Stereo and 2 Mics at every side */
        else if (CVC->tone_stereo == 1 && task->two_mic == 1)
        {
            mic_source_a = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A);
            mic_source_b = StreamAudioSource(AUDIO_HARDWARE_DIGITAL_MIC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_B);
            ...
            SourceConfigure(mic_source_b, STREAM_DIGITAL_MIC_INPUT_GAIN, (linput_gain_l & 0xf));
            SourceConfigure(mic_source_c, STREAM_DIGITAL_MIC_INPUT_GAIN, (linput_gain_r & 0xf));
            SourceConfigure(mic_source_d, STREAM_DIGITAL_MIC_INPUT_GAIN, (linput_gain_r & 0xf));
         }
#else /* Analogue mics*/
		if (CVC->tone_stereo == 0 && task->two_mic == 0)
		{
            mic_source_a = StreamAudioSource( AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A );
            SourceConfigure(mic_source_a, STREAM_CODEC_INPUT_GAIN, (linput_gain_l & 0xf));
            PRINT(("HEP:Set audio gains ANALOGUE_L0[%x]  \n", linput_gain_l));
		}     
		else if (CVC->tone_stereo == 0 && task->two_mic == 1)
		{
            mic_source_a = StreamAudioSource( AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A );
            mic_source_b = StreamAudioSource( AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_B );
            SourceConfigure(mic_source_a, STREAM_CODEC_INPUT_GAIN, (linput_gain_l & 0xf));
            SourceConfigure(mic_source_b, STREAM_CODEC_INPUT_GAIN, (linput_gain_l & 0xf));
            PRINT(("HEP:Set audio gains ANALOGUE_L0[%x] ANALOGUE_L1[%x] \n", linput_gain_l , linput_gain_r));
		}
		...
		else if (CVC->tone_stereo == 1 && task->two_mic == 0)
		{
            ...
            mic_source_b = StreamAudioSource( AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_B );
            SourceConfigure(mic_source_a, STREAM_CODEC_INPUT_GAIN, (linput_gain_l & 0xf));
            SourceConfigure(mic_source_b, STREAM_CODEC_INPUT_GAIN, (linput_gain_r & 0xf));    
            PRINT(("HEP:Set audio gains ANALOGUE_L0[%x] ANALOGUE_R0[%x] \n", linput_gain_l , linput_gain_r));
		}
		else
			PRINT(("HEP:Set ANALOGUE audio gains failed! Not support HEP_BA & 2 MICS.\n"));
#endif
            ...
            switch ( m->id ) 
            {
                ...
                case CVC_READY_MSG:
                {
                    ...
                    switch(task->cvc_plugin_variant)
                    {
                        ...
                        case DSP_HEARING:
                            PRINT(("CVC: CVC_LOADPARAMS_MSG pskey[%x] mode[%x]\n",HEP_PROFILE_PS_BASE,CVC_SR_WB));
                            CVC->Hearing_Busy_Flag = TRUE;
                            KalimbaSendMessage(CVC_LOADPARAMS_MSG, (uint16)(CVC->profile)*2+HEP_PROFILE_PS_BASE, 0, 0, 0);
                        break;
                ...
                case CVC_CODEC_MSG:
                {
					...
					lOutput_gain = m->a;
					lInput_gain_l = (CVC->HearingMode) ? *(T_mic_gain*)&m->c : *(T_mic_gain*)&m->b;
					lInput_gain_r = (CVC->HearingMode) ? *(T_mic_gain*)&m->d : *(T_mic_gain*)&m->c;
					if (!CVC->HearingMode)
						CvcCodecMessage (task, lInput_gain_l,lInput_gain_r, lOutput_gain );	
					else
						HearingCodecMessage (task, lInput_gain_l,lInput_gain_r, lOutput_gain );	                   
				...
				case HEP_PARAMS_LOADED_MSG:
				{
					if ( m->a ) /*if (m->a) the parameters are correct otherwise wrong*/
					{
						PRINT(("HEP: Parameters Loaded!\n"));
						/* Sends an event massege back to theSink.task to play a profile tone if profile load was successful */
						MessageSend ( CVC->app_task , EventSysSubwooferCloseMedia, 0 ) ;
					}
					else
					{
						PRINT(("HEP: Parameters NOT Loaded!\n"));
					}
...
void AlangoPluginChangeParamsHearing( CvcPluginTaskdata *task )
{     
   PRINT(("CSR_CVC_COMMON: AlangoPluginChangeParamsHearing\n"));
   PRINT(("HEP Busy: %x\n",(uint16)CVC->Hearing_Busy_Flag ));
   if ( CVC && CVC->HearingMode && (CVC->Hearing_Busy_Flag == FALSE))
   {
       SetAudioBusy((TaskData*) task);
  
	   CVC->profile = theSink.HearPhones_conf->Hearing.profile;
	   CVC->EQ		= theSink.HearPhones_conf->Hearing.EQ;
	   CVC->EQ_Load_Flag = CVC->Num_Of_EQ ? TRUE : FALSE;
       CVC->Hearing_Busy_Flag = TRUE;
       PRINT(("HEP ID: %x\n",(uint16)(CVC->profile)*2+HEP_PROFILE_PS_BASE));
