...
void AudioDisconnect( void )
{
	...
	{
        AUD_DEBUG(("AUD: AudioDisconnect ignored, not connected\n"));
	}
...
{
    AUD_DEBUG(("AUDIO: STOP tone, busy = %x\n",(uint16)AUDIO->AUDIO_BUSY));
	if(AUDIO->AUDIO_BUSY)
	{	
...
{   
    ...
    if (AUDIO->plugin) 
	{
...
#endif                
        {
            AUD_DEBUG(("AUD play Audio Prompt via DSP mixing\n"));				
...
#endif           
        {          
            AUD_DEBUG(("AUD play Audio Prompt - plugin loading\n"));	
            MessageSendConditionally ( plugin , AUDIO_PLUGIN_PLAY_AUDIO_PROMPT_MSG , message , (const uint16 *)AudioBusyPtr() ) ;           
        }
        ...
        else
        {
            ...
            AUD_DEBUG(("AUD play -Audio Prompt disconnect audio\n"));				
    ...
    {
        AUD_DEBUG(("AUD play Audio Prompt no plugin\n"));				
        /* if this voice prompt is set to play immediately, cancel any queued prompts */
        if(override)
            MessageCancelAll( plugin, AUDIO_PLUGIN_PLAY_AUDIO_PROMPT_MSG);
...
void AudioStopAudioPrompt ( TaskData * plugin )
{
	...
	{
        AUD_DEBUG(("AUD stop Audio Prompt plugin=%x, busy=%x\n",(uint16)plugin,(uint16)AUDIO->AUDIO_BUSY));				
