...
/*!!! DBG*/
#define TIME_DELAY_START_HEARING_AFTER_POWER_ON 4
...
/*    MAIN_DEBUG (( "HS : UE[%x]\n", id )); */
    ...
    {   
        ...
        case (EventSysPowerOnPanic):
            ...
            /*theSink.Hearing.TimeDelayStartHearingAfterPwrOn=TIME_DELAY_START_HEARING_AFTER_POWER_ON;*/
            ...
            if (theSink.HearPhones_conf->Hearing.WasActive) StartHearingDSP();
        break ;          
        case (EventUsrPowerOff):
            ...
            {
                /*Store Hearing Params from PSKEY*/
                /*{unsigned HearingParams[2]={0,0};*/
                    /* write Hearing Config */ 
					
                    /*HearingParams[0]  = theSink.Hearing.Num_Of_EQ&0xF;
                    HearingParams[0]  = HearingParams[0]<<4;
                    HearingParams[0] |= theSink.Hearing.EQ&0xF;
                    HearingParams[0]  = HearingParams[0]<<4;
                    HearingParams[0] |= theSink.Hearing.Num_Of_Profiles&0xF;
                    HearingParams[0]  = HearingParams[0]<<4;
                    HearingParams[0] |= theSink.Hearing.profile&0xF;
				
                    HearingParams[1]  = theSink.Hearing.volume&0x0F;
                    HearingParams[1] = HearingParams[1]<<1;
                    
                    MAIN_DEBUG(("HS : ************************\n"));                    
                    MAIN_DEBUG(("HS : profile=%d\n",theSink.Hearing.profile));                    
                    MAIN_DEBUG(("HS : Num_Of_Profiles=%d\n",theSink.Hearing.Num_Of_Profiles));                    
                    MAIN_DEBUG(("HS : EQ=%d\n",theSink.Hearing.EQ));                    
                    MAIN_DEBUG(("HS : Num_Of_EQ=%d\n",theSink.Hearing.Num_Of_EQ));                    
                    MAIN_DEBUG(("HS : volume=%d\n",theSink.Hearing.volume));                    
                    MAIN_DEBUG(("HS : ActiveHearing=%d\n",theSink.Hearing.ActiveHearing));                    
                    MAIN_DEBUG(("HS : ************************\n"));                                                           
                       
                    if (theSink.Hearing.ActiveHearing) HearingParams[1] |=1;     
                    MAIN_DEBUG(("HS: Store HearingParams [1]->0x%x [0]->0x%x \n",HearingParams[1],HearingParams[0]));
                    if (PsStore(CONFIG_HEARPHONES, &HearingParams, 2) == 1)
                    {
                        MAIN_DEBUG(("HS : Wrote HearingParams to PS\n"));
                    }
                }*/
				configManagerWriteHearPhonesData () ;
				
				
                if (theSink.HearPhones_conf->Hearing.ActiveHearing) StopHearingDSP();
                /*theSink.Hearing.TimeDelayStartHearingAfterPwrOn=TIME_DELAY_START_HEARING_AFTER_POWER_ON;*/
...
/* when using manual audio source routing, these events can be used to switch
   between audio sources */
...
#endif 
       case EventUsrSubwooferVolumeUp:
            MAIN_DEBUG (( "HS :  Short Press Hearing EQ+\n"));
       {
       };break;
            ...
            MAIN_DEBUG (( "HS : Short Press Hearing EQ-\n"));
       {
		...
		case EventSysSubwooferCloseMedia:
		{
			MAIN_DEBUG (( "HS : Playing Hearing Profile Loaded Tone: %x\n", (uint8)0x5F + (theSink.HearPhones_conf->Hearing.profile)));
			TonesPlayTone ( 0x5F +(theSink.HearPhones_conf->Hearing.profile), TRUE , TRUE);
		}
		break;

       case EventUsrSubwooferStartInquiry:
       MAIN_DEBUG (( "HS : Short Press Hearing\n"));
       {
           if ((stateManagerGetState()==deviceA2DPStreaming)||
               (stateManagerGetState()==deviceOutgoingCallEstablish)||
               (stateManagerGetState()==deviceIncomingCallEstablish)||
               (stateManagerGetState()==deviceActiveCallSCO)||
               (stateManagerGetState()==deviceThreeWayCallWaiting)||
               (stateManagerGetState()==deviceThreeWayCallOnHold)||
               (stateManagerGetState()==deviceThreeWayMulticall)||
               (stateManagerGetState()==deviceIncomingCallOnHold)||
               (stateManagerGetState()==deviceActiveCallNoSCO))               
           {
                /*TonesPlayTone ( 0x12, TRUE , TRUE);      */       /* Tone error  12 - tone_S*/
           }
           else
           {
			   ChangeHearingProfile(next_profile);
               /*TonesPlayTone ( 0x5F +(theSink.Hearing.profile), TRUE , TRUE);*/
               MAIN_DEBUG(("HS : Profile # = %d\n",theSink.HearPhones_conf->Hearing.profile+1));
           }
       }            
       break;
       
       case EventUsrSubwooferDisconnect:
           MAIN_DEBUG (( "HS : Hearing Btn Long Press\n"));
           if (!theSink.HearPhones_conf->Hearing.ActiveHearing)
           {
               /*TonesPlayTone ( 0x12, TRUE , TRUE);      */ /* Tone error  12 - tone_S*/
			   theSink.HearPhones_conf->Hearing.WasActive = 1;
               StartHearingDSP();
           }
           else if (theSink.HearPhones_conf->Hearing.ActiveHearing)
           {
			   lIndicateEvent = FALSE; /* doesn't indicate the event on Leds*/
               StopHearingDSP();
			   theSink.HearPhones_conf->Hearing.WasActive = 0;
           }
       break;            
...
int main(void)
{
    ...
    /*!!! AUTO POWER ON*/
/*    MessageSendLater( &theSink.task , EventUsrPowerOn , 0 ,D_SEC(3));          */
	MessageSinkTask(StreamUartSink(),&theSink.task);
